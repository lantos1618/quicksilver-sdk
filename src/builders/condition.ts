...
import { Action as ActionClass } from './action';
...
export class ConditionBuilder {
   ...
   */
  then(...actions: (Action | ActionClass)[]): this {
    if (this.conditions.length === 0) {
      throw new Error("You must call '.when()' before '.then()'.");
    }
    
    // Convert ActionClass instances to Action interfaces
    const convertedActions = actions.map(action => 
      action instanceof ActionClass ? action : action
    );
   ...
   */
  otherwise(...actions: (Action | ActionClass)[]): this {
    const convertedActions = actions.map(action => 
      action instanceof ActionClass ? action : action
    );
